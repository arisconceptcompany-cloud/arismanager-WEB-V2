...
try:
    ...
    stdin, stdout, stderr = ssh.exec_command('cat /etc/nginx/sites-enabled/apiv2.aris-cc.com')
    current_config = stdout.read().decode()
    
    print("=== Configuration actuelle ===")
    print(current_config)
    
    # Nouvelle configuration avec CORS pour HTTP et HTTPS
    new_config = '''server {
    listen 80;
...
server {
    ...
    location ~ ^/api/ {
        ...
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "https://arismanager.aris-cc.com" always;
            add_header Access-Control-Allow-Origin "http://arismanager.aris-cc.com" always;
        ...
        # CORS headers for actual responses - allow both HTTP and HTTPS
        set $cors_origin "";
        if ($http_origin ~* "^https?://arismanager\\.aris-cc\\.com$") {
